...
class TestModelDisplay:
    async def test_codex_footer_shows_selected_model(self) -> None:
        body = await _run(
            backend="codex",
            mode="on",
            model="gpt-6",
            codex_line="Codex · pro subscription (owner@example.com)\n5h  remaining 80%",
            statusline=None,
        )
        assert body is not None
        assert "Model: gpt-6" in body
        assert "owner@example.com" in body
        assert "remaining 80%" in body

    async def test_claude_footer_shows_selected_model_without_statusline(self) -> None:
        body = await _run(
            backend="claude",
            mode="off",
            model="opus",
    ...
    async def test_default_model_is_not_presented_as_a_resolved_model(self) -> None:
        body = await _run(
            backend="codex",
            mode="on",
            model="",
        ...
        assert body is not None
        assert "Model: CLI default" in body
